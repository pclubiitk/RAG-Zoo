...
def test_chunking_with_metadata(sample_docs, sample_metadata):
    ...
    assert any("source: doc1" in chunk or "id: 001" in chunk for chunk in chunks), "doc1 metadata missing"
    assert any("source: doc2" in chunk or "id: 002" in chunk for chunk in chunks), "doc2 metadata missing"
...
def test_chunking_without_metadata(sample_docs):
    ...
    assert len(chunks) > 0, "Chunks should not be empty"
    assert all("source:" not in chunk and "id:" not in chunk for chunk in chunks), "Unexpected metadata in chunk"
