...
def test_query_decomposer_with_subquestions():
    mock_llm = MagicMock()
    mock_transformer = MagicMock(return_value=DummyBundle(subquestions=[DummySubQ("a"), DummySubQ("b")]))
...
def test_query_decomposer_without_subquestions():
    mock_llm = MagicMock()
    mock_transformer = MagicMock(return_value=DummyBundle(subquestions=None, query_str="main"))
    decomposer = QueryDecomposer(mock_llm)
    decomposer.transformer = mock_transformer
