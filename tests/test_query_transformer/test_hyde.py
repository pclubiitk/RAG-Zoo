...
def test_hyde_transform_returns_custom_embedding_strs():
    ...
    mock_llm = MagicMock()
    mock_transformer = MagicMock(return_value=DummyBundle(custom_embedding_strs=["foo", "bar"], query_str="baz"))
...
def test_hyde_transform_returns_query_str_when_no_custom_embedding_strs():
    mock_llm = MagicMock()
    mock_transformer = MagicMock(return_value=DummyBundle(custom_embedding_strs=None, query_str="baz"))
    hyde = HyDe(mock_llm)
    hyde.transformer = mock_transformer
...
def test_hyde_transform_calls_transformer_with_query():
    mock_llm = MagicMock()
    mock_transformer = MagicMock(return_value=DummyBundle(custom_embedding_strs=["foo"], query_str="baz"))
    hyde = HyDe(mock_llm)
    hyde.transformer = mock_transformer
